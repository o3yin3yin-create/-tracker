        ...
        <div className="w-full p-6 flex flex-col mb-8 transition-colors duration-500" style={{ backgroundColor: themeColor, borderRadius: '28px', color: '#000000' }}>
          ...
          {isEditingMission ? (
            <input 
              ...
              className="text-3xl font-extrabold tracking-tighter outline-none bg-transparent mb-6 pb-1 border-b border-black/20"
              style={{ color: '#000000' }} placeholder="What's your goal?"
            />
          ) : (
            mission && (
              <h2 onClick={() => setIsEditingMission(true)} className="text-3xl font-extrabold tracking-tighter mb-6 cursor-pointer">
                {mission}
              </h2>
            )
          )}
